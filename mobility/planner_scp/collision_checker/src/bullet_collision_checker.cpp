...
#include <collision_checker/bullet_collision_checker.h>
...
collision_checker::BulletCollisionChecker::BulletCollisionChecker() {
  ...
  // //   get_convex_components((btCollisionObject*) env_components[i]));
  // // }

  // // convex_robot_components = get_convex_components(robot->getCollisionShape());
...
void BulletCollisionChecker::set_transformation(btCollisionObject* o, Vec3 v, Quat q) {
  o->getWorldTransform().setOrigin(btVector3(v(0), v(1), v(2)));
  o->getWorldTransform().setRotation(btQuaternion(q.x(), q.y(), q.z(), q.w()));
}

void BulletCollisionChecker::set_transformation(btCollisionObject* o, Vec3 v) {
  o->getWorldTransform().setOrigin(btVector3(v(0), v(1), v(2)));
}

std::vector<btCollisionObject*> BulletCollisionChecker::get_convex_components(btCollisionShape* cs) {
  btTransform tr;
  tr.setIdentity();
  return get_convex_components(cs, tr);
}

std::vector<btCollisionObject*> BulletCollisionChecker::get_convex_components(btCollisionObject* co) {
  return get_convex_components(co->getCollisionShape(), co->getWorldTransform());
}

std::vector<btCollisionObject*> BulletCollisionChecker::get_convex_components(btCollisionShape* cs, btTransform tr) {
  ...
  if (cs->isConvex()) {
    ...
    for (int i = 0; i < num_children; i++) {
      ...
      // TODO(acauligi): convex_components.push_back(get_convex_components(cm->getChildShape(i), comp));
    }
  }
...
bool BulletCollisionChecker::is_free_state(Vec3 v, Quat q) {
  set_transformation(robot, v, q);
  BinaryCollisionCallback result;
  cw->contactTest(robot, result);
  return !result.is_collision;
}

bool BulletCollisionChecker::is_free_motion(Vec3 v, Vec3 w) {
  ...
  for (int i = 0; i < robot_compound->getNumChildShapes(); i++) {
    ...
    cw->convexSweepTest(robot_piece, tr1 * robot_compound->getChildTransform(i),
...
bool BulletCollisionChecker::is_free_motion(Vec3 v, Quat vrot, Vec3 w, Quat wrot) {
  bool free_motion = true;
  btCompoundShape* robot_compound = reinterpret_cast<btCompoundShape*>(robot->getCollisionShape());
...
void BulletCollisionChecker::set_margin(btCollisionObject* obj, decimal_t dx = 0.0) {
  set_margin(obj->getCollisionShape(), dx);
}

void BulletCollisionChecker::set_margin(btCollisionShape* cs, decimal_t dx = 0.0) {
  if (cs->isConvex()) {
    cs->setMargin(dx);
  } else if (cs->isCompound()) {
    btCompoundShape* cm = reinterpret_cast<btCompoundShape*>cs;
    int num_children = cm->getNumChildShapes();
    for (int i = 0; i < num_children; i++) {
      set_margin(cm->getChildShape(i), dx);
    }
  }
}
...
void BulletCollisionChecker::SetContactDistance(float dist) {
  ...
  btCollisionObjectArray& objs = cw->getCollisionObjectArray();
  for (int i = 0; i < objs.size(); ++i) {
    objs[i]->setContactProcessingThreshold(dist);
  }
  btCollisionDispatcher* dispatcher = static_cast<btCollisionDispatcher*>(cw->getDispatcher());
...
void compute_distance(const btCollisionShape* cs1, const btTransform& tr1,
  ...
  if (cs1->isConvex() && cs2->isConvex()) {
    ...
    btGjkPairDetector convexConvex(reinterpret_cast<btConvexShape*>(cs1),
                                   reinterpret_cast<btConvexShape*>(cs2),
    ...
    btCompoundShape* com1 = reinterpret_cast<btCompoundShape*>(cs1);
    ...
    btCompoundShape* com2 = reinterpret_cast<btCompoundShape*>(cs2);
...
void compute_weighted_distance(const btCollisionShape* cs1, const btTransform& tr1,
  ...
  if (cs1->isConvex() && cs2->isConvex()) {
    btVoronoiSimplexSolver sGjkSimplexSolver;
    sGjkSimplexSolver.setEqualVertexThreshold(0.f);
    btWeightedGjkPairDetector convexConvex(reinterpret_cast<btConvexShape*>(cs1),
                                           reinterpret_cast<btConvexShape*>(cs2),
                                           &sGjkSimplexSolver, U);
    btGjkPairDetector::ClosestPointInput input;
    btPointCollector output;
