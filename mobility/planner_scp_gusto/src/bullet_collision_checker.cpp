...
scp::BulletCollisionChecker::BulletCollisionChecker() {
...
void BulletCollisionChecker::SetTransformation(btCollisionObject* o, Vec3 v, Quat q) {
  o->getWorldTransform().setOrigin(btVector3(v(0), v(1), v(2)));
  o->getWorldTransform().setRotation(btQuaternion(q.x(), q.y(), q.z(), q.w()));
}

void BulletCollisionChecker::SetTransformation(btCollisionObject* o, Vec3 v) {
  o->getWorldTransform().setOrigin(btVector3(v(0), v(1), v(2)));
}
...
bool BulletCollisionChecker::IsFreeState(Vec3 v, Quat q) {
...
bool BulletCollisionChecker::IsFreeMotion(Vec3 v, Vec3 w) {
...
bool BulletCollisionChecker::IsFreeMotion(Vec3 v, Quat vrot, Vec3 w, Quat wrot) {
  bool free_motion = true;
  btCompoundShape* robot_compound = reinterpret_cast<btCompoundShape*>(robot->getCollisionShape());
...
void BulletCollisionChecker::SetMargin(btCollisionObject* obj, decimal_t dx = 0.0) {
  SetMargin(obj->getCollisionShape(), dx);
}

void BulletCollisionChecker::SetMargin(btCollisionShape* cs, decimal_t dx = 0.0) {
  if (cs->isConvex()) {
    cs->setMargin(dx);
  } else if (cs->isCompound()) {
    btCompoundShape* cm = reinterpret_cast<btCompoundShape*>cs;
